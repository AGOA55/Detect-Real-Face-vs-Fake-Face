                ...
                if face_rois:
                    ...
                    for embedding, track_id in zip(batch_embeddings, face_track_ids):
                        ...
                        if is_confident and best_similarity > self.confidence_threshold:
                            ...
                            if best_similarity > 0.5:  # มี similarity บ้างแต่ไม่พอ
        ...
        while count < samples_per_angle:
            ...
            if len(faces) > 0:
                ...
                if face_img.size > 0:
                    ...
                    # เก็บรูปเฉพาะเมื่อคุณภาพดีและทุก 
...
def generate_embeddings():
    ...
    model_info = {
        'version': '2.0',
        'model_type': 'FaceNet + Advanced Augmentation',
        'stats': stats,
        'settings': {
...
# --- 3. Improved Face Recognition with Proper Tracking ---
def recognize_faces():
    try:
    ...
    print(f"\n🔄 Advanced recognition started. Processing: {'GPU+CPU' if GPU_AVAILABLE else 'CPU Only'}")
    ...
    try:
        while cap.isOpened():
                ...
                for track_id, face_idx in face_assignments.items():
                    ...
                    if landmark_idx >= 0:
                        ...
                        if person_states[track_id]['verified'] and track_id in recognition_cache:
                            result = recognition_cache[track_id]
                            
                            # เพิ่มการแสดงข้อมูล confidence gap
                            conf_gap = result.get('confidence_gap', 0)
                            display_text = f"{result['name']} ({result['confidence']:.2f}|{conf_gap:.2f})"
            ...
            cv2.putText(frame, f"Advanced Mode: {gpu_status} | Faces: {len(current_detections)}", 
                       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
...
def main():
    ...
    while True:
        ...
        print("3. Start advanced recognition (GPU+CPU+Tracking)")
